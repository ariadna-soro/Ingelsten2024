...
# Offsets (lattice parameters) for many identical GAs
gridstep1 = np.array([1, 0])
gridstep2 = np.array([2, -1])

# The different GAs are displaced by these amounts [dx,dy] from the centre of the grid:
offsets = np.array([np.array([0, 0]), gridstep1])
...
cPtsRel = np.array([[
                      [-(nRad1+mRad1)//2, (-nRad1+mRad1)//2],
                      [(nRad1-mRad1)//2, (nRad1+mRad1)//2],
                      [(nRad1+mRad1)//2, (nRad1-mRad1)//2],
                      [(-nRad1+mRad1)//2, -(nRad1+mRad1)//2]
                      ],
                    [[-(nRad2+mRad2)//2, (-nRad2+mRad2)//2],
                      [(nRad2-mRad2)//2, (nRad2+mRad2)//2],
                      [(nRad2+mRad2)//2, (nRad2-mRad2)//2],
                      [(-nRad2+mRad2)//2, -(nRad2+mRad2)//2]
                      ]])
...
# tEvOp_full = np.diag(np.ones(N*N+nAtoms, dtype='complex64'))
# for a in range(nAtoms):
#     tEvOp_full[a,a] = tEvOp_eff[0,0]
#     for p in range(nCpts):
#         cpLoc1 = couplingPts[a,p,0] + couplingPts[a,p,1]*N
#         tEvOp_full[nAtoms+cpLoc1,a] = tEvOp_eff[1,0]/np.sqrt(nCpts)
#         tEvOp_full[a,nAtoms+cpLoc1] = tEvOp_eff[0,1]/np.sqrt(nCpts)
#         tEvOp_full[nAtoms+cpLoc1,nAtoms+cpLoc1] = 1. + (tEvOp_eff[1,1]-1)/nCpts
#         for q in range(p+1,nCpts):
#             cpLoc2 = couplingPts[a,q,0] + couplingPts[a,q,1]*N
#             tEvOp_full[nAtoms+cpLoc1,nAtoms+cpLoc2] = (tEvOp_eff[1,1]-1.)/nCpts
#             tEvOp_full[nAtoms+cpLoc2,nAtoms+cpLoc1] = (tEvOp_eff[1,1]-1.)/nCpts
...
pBS1 = np.abs(stateEvol[int(tIntf/dt)+1, nAtoms:])**2
posBS1 = pBS1.argmax()+nAtoms
posBS2 = pBS1.argmax()+nAtoms+offsets[1,0]+N*offsets[1,1]
#take the most populated cavity and show the time evolution:
pBS1Exc = np.abs(stateEvol[:tiMax+1, posBS1])**2 
pBS2Exc = np.abs(stateEvol[:tiMax+1, posBS2])**2 
axs[0,1].plot(ts[:tiMax+1], pBS1Exc[:tiMax+1], linewidth=3, color='C0', label=r'$\vec{n} = (100,100)$')
axs[0,1].plot(ts[:tiMax+1], pBS2Exc[:tiMax+1], linewidth=3, color='C1', label=r'$\vec{n} = (101,100)$')
axs[0,1].set(xlabel='$tJ$', ylabel='$|C_{B}(t)|^2$')
axs[0,1].set_title(r"$\bf{(b)}$"+' Bath population at\n\n\n')

# Add legends
axs[0,0].legend(loc="lower center", bbox_to_anchor=(0.5, 0.95), frameon=False, labelspacing=0.3)
axs[0,1].legend(loc="lower center", bbox_to_anchor=(0.5, 0.95), frameon=False, labelspacing=0.3)
...
for a in range(nAtoms):
    ...
    x = np.append(x, x[0])
    y = np.append(y, y[0])
    axs[1,0].plot(x+1,y+1, marker='o', markersize=15, ls='-', lw=4, color=colorList[a])
    axs[1,1].plot(x+1,y+1, marker='o', markersize=15, ls='-', lw=4, color=colorList[a])
...
fig.tight_layout()
fig.subplots_adjust(right=0.85, top=0.78, wspace=0.4, hspace=0.5)
...
fig.savefig('/Users/ari/Dropbox/My Mac (Ariadnas-MacBook-Pro.local)/Documents/Chalmers/PhD thesis/Projects/2023 2D Structured/2-GA-DFI_Dynamics.pdf', dpi=fig.dpi, bbox_inches='tight')
